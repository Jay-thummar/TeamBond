...
import com.spring.codeamigosbackend.recommendation.services.HackathonMatcherService;
...
public class HackathonController {
    ...
    private final HackathonMatcherService hackathonMatcherService;
    ...
    public ResponseEntity<List<Hackathon>> getMyHackathons(@RequestParam String userId) {
        return ResponseEntity.ok(hackathonService.getMyHackathons(userId));
    }

    @GetMapping("/{id}/match-candidates")
    public ResponseEntity<?> matchCandidates(@PathVariable String id) {
        // 1. Fetch Hackathon
        Hackathon hackathon = hackathonService.getHackathonById(id);
        if (hackathon == null) {
            return ResponseEntity.status(404).body(Map.of("error", "Hackathon not found"));
        }
        ...
        // 3. AI Rank
        List<Map<String, Object>> rankedCandidates = hackathonMatcherService.rankCandidates(hackathon, candidates);
