...
import { getChatKeyFromIdb, setDirectoryInIdb } from "../../config/IndexDb";
...
const PersonalChatChat = ({ memberId, memberName, isKeySetupComplete }) => {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        if (err.name === 'NotFoundError' || err.message === 'DIRECTORY_MISSING' || err.message.includes('permission')) {
          setKeyError(true);
        }
  ...
  const handleRestoreAccess = async () => {
    try {
      ...
      await setDirectoryInIdb(handle);
      setKeyError(false);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <main
        ...
        className="flex-1 overflow-auto p-4 bg-slate-600 relative"
      >
        {keyError && (
          <div className="absolute inset-0 z-50 flex items-center justify-center bg-gray-900/90 backdrop-blur-sm p-4">
            <div className="bg-gray-800 p-6 rounded-xl shadow-2xl border border-red-500/50 max-w-md text-center">
              <div className="mb-4 text-red-400 text-5xl">📂</div>
              <h3 className="text-xl font-bold text-white mb-2">Secure Folder Access Lost</h3>
              <p className="text-gray-300 mb-6">
                The browser cannot access your secure key folder. This happens when permissions verify out or the folder was moved.
                <br /><br />
                Please re-select your <b>CodeAmigos--Frontend</b> (or previously selected) folder to restore chat access.
              </p>
              <button
                onClick={handleRestoreAccess}
                className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-semibold transition-all shadow-lg hover:shadow-blue-500/30"
              >
                Reconnect Secure Folder
              </button>
            </div>
          </div>
        )}
