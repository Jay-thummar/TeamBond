...
const HackathonRegistrationForm = () => {
  ...
  const validateStep2 = () => {
    ...
    const regStart = new Date(formData.registrationDates.start).getTime();
    const regEnd = new Date(formData.registrationDates.end).getTime();
    const hackStart = new Date(formData.hackathonDates.start).getTime();
    const hackEnd = new Date(formData.hackathonDates.end).getTime();

    // Removed strict future check for Registration Start Date to allow "starting now"
    if (regEnd <= now) {
